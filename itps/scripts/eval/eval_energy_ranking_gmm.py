...
def main():
    ...
    parser.add_argument("--n-noise", type=int, default=DEFAULT_ENERGY_N_NOISE,
                        help=f"Noise draws each candidate's energy is averaged over (default "
                             ...
                             f"(common random numbers), so a pair's energy difference -- the "
                             f"only thing the win rate looks at -- barely depends on which "
                             f"draws came up. Ignored with --deterministic.")
    parser.add_argument("--deterministic", action="store_true",
                        help="Score energies with no noise added; the candidate is only "
    ...
    parser.add_argument("--energy-seed", type=int, default=DEFAULT_ENERGY_SEED,
                        help=f"Seed for the energy noise draws (default {DEFAULT_ENERGY_SEED}), "
                             f"so scoring is stochastic but reproducible across runs. Drawn from "
                             f"a private generator, so it does not disturb candidate sampling.")
